...
| Component | Description |  
| ----------------- | ------------- |  
| `kubelet` | The Kubernetes agent that processes the orchestration requests from the control plane along with scheduling and running the requested containers. The heart of the worker-node. Acts like kubernetes agent running on every node - Always in close communication with the Control plane kube-scheduler.  |  
| *kube-proxy* | Handles virtual networking on each node. The proxy routes network traffic and manages IP addressing for services and pods. Maintains the network rules on the worker-nodes. |  
